...
std::string MOUNT_POINT = "/mnt/rootfs_temp";
...
std::string USERNAME = "";
const std::string EXT4_LABEL = "LIVE_SYSTEM";
...
bool formatFilesystem(const std::string& fsType, const std::string& filename) {
    ...
    std::string command = "sudo mkfs.ext4 -F -O ^has_journal,^resize_inode -E lazy_itable_init=0 -m 0 -L \"" + EXT4_LABEL + "\" " + filename;
    execute_command(command, true);
    return true;
}
...
void printFinalMessage(const std::string& outputFile) {
    std::cout << std::endl;
    std::cout << COLOR_CYAN << "Compressed SquashFS image created successfully: " << outputFile << COLOR_RESET << std::endl;
            ...
            case '\n':
                switch (selected) {
                    ...
                    case 2: {
                        ...
                        execute_command("sudo rm -rf " + outputImgPath + " " + finalImgPath, true);
                        ...
                        if (!createImageFile(sizeInput, outputImgPath) ||
                            !formatFilesystem("ext4", outputImgPath) ||
                            !mountFilesystem("ext4", outputImgPath, MOUNT_POINT) ||
                            !copyFilesWithRsync(SOURCE_DIR, MOUNT_POINT, "ext4")) {
                            break;
                        }

                        unmountAndCleanup(MOUNT_POINT);

                        // Create SquashFS from ext4 image
