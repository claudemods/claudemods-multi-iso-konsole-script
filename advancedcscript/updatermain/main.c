...
bool verify_installation(const char* path) {
    struct stat st = {0};
    if (stat(path, &st) == -1) {
        return false;
    }
...
void* execute_update_thread(void* arg) {
    ...
    printf("\033[38;2;0;255;255mChecking for existing installation...\033[0m\n");
    
    // First, remove existing installation
    printf("\033[38;2;0;255;255mRemoving old files...\033[0m\n");
    snprintf(command, sizeof(command), "echo '%s' | sudo -S rm -rf /home/$USER/claudemods-multi-iso-creator >/dev/null 2>&1", password);
    ret = system(command);
    
    snprintf(command, sizeof(command), "echo '%s' | sudo -S rm -f /usr/bin/%s >/dev/null 2>&1", password, executable_name);
    ret = system(command);

    printf("\033[38;2;0;255;255mUpdating repository...\033[0m\n");
    ...
    if (strcmp(detected_distro, "arch") == 0) {
        printf("\033[38;2;0;255;255mRebuilding Arch version...\033[0m\n");
        snprintf(command, sizeof(command), "cd /home/$USER/claudemods-multi-iso-creator/advancedcscript/arch && make clean >/dev/null 2>&1 && make >/dev/null 2>&1");
        ret = system(command);
        if (ret == 0) {
        ...
        printf("\033[38;2;0;255;255mUpdating Arch build images...\033[0m\n");
        snprintf(command, sizeof(command), "echo '%s' | sudo -S rm -rf /home/$USER/.config/cmi/arch >/dev/null 2>&1", password);
        system(command);
        ...
        ret = system(command);
        if (ret == 0 && verify_installation("/home/$USER/.config/cmi/arch")) {
            printf("\033[38;2;0;255;0mArch build images updated successfully!\033[0m\n");
        }
    }
    else if (strcmp(detected_distro, "ubuntu") == 0) {
        printf("\033[38;2;0;255;255mRebuilding Ubuntu version...\033[0m\n");
        snprintf(command, sizeof(command), "cd /home/$USER/claudemods-multi-iso-creator/advancedcscript/ubuntu/noble && make clean >/dev/null 2>&1 && make >/dev/null 2>&1");
        ret = system(command);
        if (ret == 0) {
        ...
        printf("\033[38;2;0;255;255mUpdating Ubuntu build images...\033[0m\n");
        snprintf(command, sizeof(command), "echo '%s' | sudo -S rm -rf /home/$USER/.config/cmi/ubuntu >/dev/null 2>&1", password);
        system(command);
        ...
        ret = system(command);
        if (ret == 0 && verify_installation("/home/$USER/.config/cmi/ubuntu")) {
            printf("\033[38;2;0;255;0mUbuntu build images updated successfully!\033[0m\n");
        }
    }
    else if (strcmp(detected_distro, "debian") == 0) {
        printf("\033[38;2;0;255;255mRebuilding Debian version...\033[0m\n");
        snprintf(command, sizeof(command), "cd /home/$USER/claudemods-multi-iso-creator/advancedcscript/debian/bookworm && make clean >/dev/null 2>&1 && make >/dev/null 2>&1");
        ret = system(command);
        if (ret == 0) {
        ...
        printf("\033[38;2;0;255;255mUpdating Debian build images...\033[0m\n");
        snprintf(command, sizeof(command), "echo '%s' | sudo -S rm -rf /home/$USER/.config/cmi/debian >/dev/null 2>&1", password);
        system(command);
        ...
        ret = system(command);
        if (ret == 0 && verify_installation("/home/$USER/.config/cmi/debian")) {
            printf("\033[38;2;0;255;0mDebian build images updated successfully!\033[0m\n");
        }
    }
...
void show_loading_bar() {
    ...
    printf("\033[38;2;0;255;0mClaudeMods Multi ISO Creator Updater 21-06-2025\033[0m\n");
    printf("\033[38;2;0;255;0mUpdating: \033[0m\n");
    printf("[");
...
int main() {
    ...
    printf("\033[38;2;0;255;0mUpdate complete!\033[0m\n");
    printf("\033[38;2;0;255;0mThe executable has been updated at /usr/bin/\033[0m\n");
    printf("\033[38;2;0;255;0mConfiguration files updated in ~/.config/cmi/\033[0m\n");
