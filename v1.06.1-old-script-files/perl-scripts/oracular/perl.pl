...
sub run_squashfs_command {
    ...
    run_command("./oracularSquashFS-Creator.bin");
...
sub convert_to_dwarfs {
    ...
    run_command("./oracularDwarfs.bin");
...
sub create_iso {
    ...
    my $build_image_dir = File::Spec->catfile($application_dir_path, "build-files/build-image-oracular");

    progress_dialog("Creating ISO...");

    my $iso_file_name = "$iso_name\_amd64_" . strftime("%Y-%m-%d_%H%M", localtime) . ".iso";
    my $xorriso_command = "xorriso -as mkisofs -o $iso_file_name -V 2024 -iso-level 3 -isohybrid-mbr /usr/lib/ISOLINUX/isohdpfx.bin -c isolinux/boot.cat -b isolinux/isolinux.bin -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e boot/grub/efiboot.img -no-emul-boot -isohybrid-gpt-basdat build-files/build-image-oracular";
...
sub change_boot_artwork {
    ...
    my $application_dir_path = abs_path();
    my $destination_dir = File::Spec->catfile($application_dir_path, "build-files/build-image-oracular", "isolinux");
...
sub generate_initramfs {
    ...
    my $initramfs_command = "sudo mkinitramfs -o build-files/build-image-oracular/live/initrd.img-$kernel_version $kernel_version";
    my $copy_kernel_command = "sudo cp /boot/vmlinuz build-files/build-image-oracular/live/vmlinuz-$kernel_version";
...
sub copy_vmlinuz {
    ...
    my $application_dir_path = abs_path();
    my $destination_dir = File::Spec->catfile($application_dir_path, "build-files/build-image-oracular", "live");
...
sub configuration_options_menu {
    while (1) {
        ...
        if ($choice eq '1') {
            run_command("kate /build-files/build-image-oracular/boot/grub/grub.cfg");
        } elsif ($choice eq '2') {
            run_command("kate /build-files/build-image-oracular/isolinux/isolinux.cfg");
        } elsif ($choice eq '3') {
            change_boot_artwork();
        } elsif ($choice eq '4') {
            generate_initramfs();
...
sub display_help {
    ...
    print colored("2. ClaudeMods oracular ISO Creator Script - Enter Arch commands.", 'blue') . "\n";
...
sub main_menu {
    ...
    while (1) {
        ...
        print colored("2. ClaudeMods oracular ISO Creator Script", 'blue') . "\n";
...
sub arch_iso_creator_menu {
    while (1) {
        print "ClaudeMods oracular ISO Creator Menu:\n";
        ...
        if ($choice =~ /^\s*1\s*$/) {
            run_command("kate /build-files/build-image-oracular/system_info.txt");
        } elsif ($choice =~ /^\s*2\s*$/) {
            show_information();
        } elsif ($choice =~ /^\s*3\s*$/) {
...
sub handle_arch_commands {
    while (1) {
        print "Available oracular Commands:\n";
