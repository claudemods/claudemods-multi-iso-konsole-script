...
sub run_squashfs_command {
    ...
    run_command("./DebianSquashFS-Creator.bin");
...
sub create_iso {
    ...
    my $build_image_dir = File::Spec->catfile($application_dir_path, "build-files/build-image-debian");

    # Ensure the build image directory exists
    if (!-d $build_image_dir) {
        error_box("Directory Error", "The build image directory does not exist.");
        return;
    }
    ...
    my $iso_file_path = File::Spec->catfile($application_dir_path, $iso_file_name);

    my $xorriso_command = "xorriso -as mkisofs -o $iso_file_path -V 2024 -iso-level 3 -isohybrid-mbr /usr/lib/ISOLINUX/isohdpfx.bin -c isolinux/boot.cat -b isolinux/isolinux.bin -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e boot/grub/efiboot.img -no-emul-boot -isohybrid-gpt-basdat $build_image_dir";
...
sub show_information {
    my $application_dir_path = abs_path();
    my $information_file_path = File::Spec->catfile($application_dir_path, "informationdebian.txt");
...
sub change_boot_artwork {
    ...
    my $application_dir_path = abs_path();
    my $destination_dir = File::Spec->catfile($application_dir_path, "build-files/build-image-debian", "isolinux");
...
sub generate_initramfs {
    ...
    my $initramfs_command = "sudo mkinitramfs -o build-files/build-image-debian/live/initrd.img-$kernel_version $kernel_version";
    my $copy_kernel_command = "sudo cp /boot/vmlinuz build-files/build-image-debian/live/vmlinuz-$kernel_version";
...
sub copy_vmlinuz {
    ...
    my $application_dir_path = abs_path();
    my $destination_dir = File::Spec->catfile($application_dir_path, "build-files/build-image-debian", "live");
...
sub configuration_options_menu {
    while (1) {
        ...
        if ($choice eq '1') {
            run_command("kate /build-files/build-image-debian/boot/grub/grub.cfg");
        } elsif ($choice eq '2') {
            run_command("kate /build-files/build-image-debian/isolinux/isolinux.cfg");
        } elsif ($choice eq '3') {
            change_boot_artwork();
        } elsif ($choice eq '4') {
            generate_initramfs();
...
sub display_help {
    ...
    print colored("2. ClaudeMods Debian ISO Creator Script - Enter Arch commands.", 'blue') . "\n";
...
sub main_menu {
    ...
    while (1) {
        ...
        print colored("2. ClaudeMods Debian ISO Creator Script", 'blue') . "\n";
...
sub arch_iso_creator_menu {
    while (1) {
        print "ClaudeMods Debian ISO Creator Menu:\n";
        ...
        if ($choice =~ /^\s*1\s*$/) {
            run_command("kate /build-files/build-image-debian/system_info.txt");
        } elsif ($choice =~ /^\s*2\s*$/) {
            show_information();
        } elsif ($choice =~ /^\s*3\s*$/) {
