#ifndef UBUNTUISOCREATOR_H
#define UBUNTUISOCREATOR_H
...
void print_banner() {
    ...
    "░╚════╝░╚══════╝╚═╝░░░░░░╚═════╝░╚═════╝░╚══════╝╚═╝░░░░░╚═╝░╚════╝░╚═════╝░╚═════╝░\n";
    std::cout << RESET;
    std::cout << RED << "Claudemods Ubuntu ISO Creator Advanced C++ Script v1.01 21-06-2025" << RESET << std::endl;
...
void generate_initrd_ubuntu() {
    progress_dialog("Generating Initramfs (Ubuntu)...");
...
void edit_grub_cfg_ubuntu() {
    progress_dialog("Opening grub.cfg (ubuntu)...");
...
void edit_isolinux_cfg_ubuntu() {
    progress_dialog("Opening isolinux.cfg (ubuntu)...");
...
void squashfs_menu() {
    ...
    int key;
    while (true) {
        key = show_menu("SquashFS Creator", items, selected);
        switch (key) {
            case 'A':
...
void create_iso() {
    ...
    std::string build_image_dir = std::string(application_dir_path) + "/build-image-noble";
...
void iso_creator_menu() {
    ...
    int key;
    while (true) {
        key = show_menu("ISO Creator Menu", items, selected);
        switch (key) {
            case 'A':
                if (selected > 0) selected--;
                break;
...
void command_installer_menu() {
    ...
    int key;
    while (true) {
        key = show_menu("Command Installer Menu", items, selected);
        switch (key) {
            case 'A':
                if (selected > 0) selected--;
                break;
